...
[MontageFile,MontageDir] = uigetfile('*.mat','Pick the montage file');
if MontageDir == 0
    return;
end
...
%InfoForGridAutoFocus_FileNameStr = sprintf('%s\\MontageOverviewImage.mat',MontageDir);
%load(InfoForGridAutoFocus_FileNameStr);
load(fullfile(MontageDir,MontageFile));


X = Info.StageX_Meters;
Y = Info.StageY_Meters;
Z = Info.WorkingDistance;
