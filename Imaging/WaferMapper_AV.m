function varargout = WaferMapper_AV(varargin)
% WAFERMAPPER_AV MATLAB code for WaferMapper_AV.fig
%      WAFERMAPPER_AV, by itself, creates a new WAFERMAPPER_AV or raises the existing
%      singleton*.
%
%      H = WAFERMAPPER_AV returns the handle to a new WAFERMAPPER_AV or the handle to
%      the existing singleton*.
%
%      WAFERMAPPER_AV('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WAFERMAPPER_AV.M with the given input arguments.
%
%      WAFERMAPPER_AV('Property','Value',...) creates a new WAFERMAPPER_AV or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before WaferMapper_AV_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to WaferMapper_AV_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help WaferMapper_AV

% Last Modified by GUIDE v2.5 13-Mar-2013 09:29:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @WaferMapper_AV_OpeningFcn, ...
                   'gui_OutputFcn',  @WaferMapper_AV_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before WaferMapper_AV is made visible.
function WaferMapper_AV_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to WaferMapper_AV (see VARARGIN)

% Choose default command line output for WaferMapper_AV
...
%Make sure there is not another instance of WaferMapper_AV running
...
%NOTE: The following renaming with a ' - ' after the WaferMapper_AV is to allow findobj
...
if strcmp(ButtonName,'OK')
    ...
    if GuiGlobalsStruct.IsZeissAPIInitialized
        ...
        h_msgbox = msgbox('Programatically setting Zeiss SEM to 30x before init of FIBICS. This is necessary to maintain consistency of FOV calcs. between FIBICS sessions','modal');
        ...
        if GuiGlobalsStruct.MyCZEMAPIClass.GetMag() == StartMag
            ...
            %bring the WaferMapper_AV figure to the front again
...
function varargout = WaferMapper_AV_OutputFcn(hObject, eventdata, handles) 
...
if ~exist(GuiGlobalsStruct.UTSLDirectory, 'dir')
    MyStr = sprintf('Could not find UTSL directory: %s.',GuiGlobalsStruct.UTSLDirectory);
    uiwait(msgbox(MyStr));
    set(handles.NewUTSL_MenuItem,'Enable','on');
    set(handles.OpenUTSL_MenuItem,'Enable','on');
    return;
    ...
    for i=1:length(DirListArray)
        if (DirListArray(i).isdir == 1) && (DirListArray(i).name(1) ~= '.')
            if ~strcmp(DirListArray(i).name, 'AlignedTargetListsDirectory')
                ListOfDirNames{n} = DirListArray(i).name;
                n = n + 1;
            end
        end
...
if exist(GuiGlobalsStruct.SectionOverviewTemplateDirectory, 'dir')
    MyStr = sprintf('%s already exists. If you are sure you want to choose a new section template image then manually delete the directory.',...
...
DirListArray = dir(GuiGlobalsStruct.SectionOverviewsDirectory);
n = 1;
ListOfTifFileNames = [];
for i=1:length(DirListArray)
...
if length(ListOfTifFileNames) > 0
    ...
    if isok == 1
        ImageFileNameStr = sprintf('%s\\%s',GuiGlobalsStruct.SectionOverviewsDirectory,ListOfTifFileNames{SelectionNumber});  
    else
        return;
    end
else
...
if exist(GuiGlobalsStruct.SectionOverviewsAlignedWithTemplateDirectory, 'dir')
    MyStr = sprintf('%s already exists. If you are sure you want to overwrite then manually delete the directory.',...
...
%GenerateListOfAlignedTargetPoints_old; %Changed by AV on 11/20/2012
...
% handles    structure with handles and user data (see GUIDATA)
global GuiGlobalsStruct;
uiwait(MontageParametersGUI_AV()); % Added by AV 3/12/13
...
SavedMontageParameters.TileFOV_microns = GuiGlobalsStruct.MontageParameters.TileFOV_microns;
SavedMontageParameters.TileWidth_pixels = GuiGlobalsStruct.MontageParameters.TileWidth_pixels;
SavedMontageParameters.TileDwellTime_microseconds = GuiGlobalsStruct.MontageParameters.TileDwellTime_microseconds;

SavedMontageParameters.MontageNorthAngle = GuiGlobalsStruct.MontageParameters.MontageNorthAngle;
SavedMontageParameters.NumberOfTileRows = GuiGlobalsStruct.MontageParameters.NumberOfTileRows;
SavedMontageParameters.NumberOfTileCols = GuiGlobalsStruct.MontageParameters.NumberOfTileCols;
SavedMontageParameters.PercentTileOverlap = GuiGlobalsStruct.MontageParameters.PercentTileOverlap;
SavedMontageParameters.XOffsetFromAlignTargetMicrons = GuiGlobalsStruct.MontageParameters.XOffsetFromAlignTargetMicrons;
SavedMontageParameters.YOffsetFromAlignTargetMicrons = GuiGlobalsStruct.MontageParameters.YOffsetFromAlignTargetMicrons;

SavedMontageParameters.AF_X_Offset_Microns = GuiGlobalsStruct.MontageParameters.AF_X_Offset_Microns;
SavedMontageParameters.AF_Y_Offset_Microns = GuiGlobalsStruct.MontageParameters.AF_Y_Offset_Microns;

SavedMontageParameters.AutoFocusStartMag = GuiGlobalsStruct.MontageParameters.AutoFocusStartMag;
SavedMontageParameters.IsPerformQualityCheckOnEveryAF = GuiGlobalsStruct.MontageParameters.IsPerformQualityCheckOnEveryAF;
SavedMontageParameters.AFQualityThreshold = GuiGlobalsStruct.MontageParameters.AFQualityThreshold;
SavedMontageParameters.IsPerformQualCheckAfterEachImage = GuiGlobalsStruct.MontageParameters.IsPerformQualCheckAfterEachImage;
SavedMontageParameters.ImageQualityThreshold = GuiGlobalsStruct.MontageParameters.ImageQualityThreshold;


SavedMontageParameters.IsSingle_AF_ForWholeMontage = GuiGlobalsStruct.MontageParameters.IsSingle_AF_ForWholeMontage;
SavedMontageParameters.IsSingle_AFASAF_ForWholeMontage = GuiGlobalsStruct.MontageParameters.IsSingle_AFASAF_ForWholeMontage;
SavedMontageParameters.IsAFOnEveryTile = GuiGlobalsStruct.MontageParameters.IsAFOnEveryTile;
SavedMontageParameters.IsAFASAFOnEveryTile = GuiGlobalsStruct.MontageParameters.IsAFASAFOnEveryTile;
SavedMontageParameters.IsPlaneFit = GuiGlobalsStruct.MontageParameters.IsPlaneFit;
SavedMontageParameters.RowDistBetweenAFPointsMicrons = GuiGlobalsStruct.MontageParameters.RowDistBetweenAFPointsMicrons;
SavedMontageParameters.ColDistBetweenAFPointsMicrons = GuiGlobalsStruct.MontageParameters.ColDistBetweenAFPointsMicrons;
SavedMontageParameters.AutofunctionScanrate = GuiGlobalsStruct.MontageParameters.AutofunctionScanrate;
SavedMontageParameters.IsSingle_AFAS_ForFirstTile = GuiGlobalsStruct.MontageParameters.IsSingle_AFAS_ForFirstTile; % Added by AV 3/13/13



SavedMontageParameters.IsAcquireOverviewImage = GuiGlobalsStruct.MontageParameters.IsAcquireOverviewImage;
SavedMontageParameters.MontageOverviewImageFOV_microns = GuiGlobalsStruct.MontageParameters.MontageOverviewImageFOV_microns;
SavedMontageParameters.MontageOverviewImageWidth_pixels = GuiGlobalsStruct.MontageParameters.MontageOverviewImageWidth_pixels;
SavedMontageParameters.MontageOverviewImageHeight_pixels = GuiGlobalsStruct.MontageParameters.MontageOverviewImageHeight_pixels;
SavedMontageParameters.MontageOverviewImageDwellTime_microseconds = GuiGlobalsStruct.MontageParameters.MontageOverviewImageDwellTime_microseconds;
...
if isfield(SavedMontageParameters, 'IsSingle_AF_ForWholeMontage')
    ...
    GuiGlobalsStruct.MontageParameters.IsSingle_AFAS_ForFirstTile = SavedMontageParameters.IsSingle_AFAS_ForFirstTile; % Added by AV 3/13/13
end
...
GuiGlobalsStruct.MontageParameters.MontageOverviewImageHeight_pixels = SavedMontageParameters.MontageOverviewImageHeight_pixels;
