...
% Augmente DBX cells with cells computed from DBXpop.m

load('LeadLikeDBX.mat');
load('LagLikeDBX.mat');

AugmentedCells = [DbxCells,LeadLikeDBX',LagLikeDBX'];

for i = 1:numel(AugmentedCells)
    AugDBX(i) = InputsByClass(AugmentedCells(i),df);
end

leadNeurons = [LeadLikeDBX',leadNeurons];
lagNeurons = [LagLikeDBX',leadNeurons];

%% Saccadic module

UniqueSaccadicAxons = unique(vertcat(AugDBX.Saccadic));
    ix1 = 1;
    ix2 = 1;
...
for i = 1:numel(AugmentedCells)
    tempLeadIDs = find(ismember(AugDBX(i).Inputs, [Lead.SaccadeAxonID]));
    tempLagIDs = find(ismember(AugDBX(i).Inputs, [Lag.SaccadeAxonID]));
    AugDBX(i).LeadSaccadicPathLength = AugDBX(i).PathLength(tempLeadIDs)/max(Pvec_tree(AugDBX(i).Tree{1}));
    AugDBX(i).LeadSaccadicPSD = AugDBX(i).PSDsize(tempLeadIDs);
    LeadSaccadicPathLengths(i,:) = histcounts(AugDBX(i).LeadSaccadicPathLength,0:0.1:1);
    
    AugDBX(i).LagSaccadicPathLength = AugDBX(i).PathLength(tempLagIDs)/max(Pvec_tree(AugDBX(i).Tree{1}));
    AugDBX(i).LagSaccadicPSD = AugDBX(i).PSDsize(tempLagIDs);
    LagSaccadicPathLengths(i,:) = histcounts(AugDBX(i).LagSaccadicPathLength,0:0.1:1);    
...
subplot(4,4,2)
histogram(vertcat(AugDBX.LeadSaccadicPathLength),10,'FaceColor',leadColor,'EdgeColor','none');
hold on;
histogram(vertcat(AugDBX.LagSaccadicPathLength),10,'FaceColor',lagColor,'EdgeColor','none');
axis square;
box off;
...
UniqueVestibularAxons = unique(vertcat(AugDBX.Vestibular));
    ix1 = 1;
    ix2 = 1;  
...
for i = 1:numel(AugmentedCells)
    tempLeadIDs = find(ismember(AugDBX(i).Inputs, [Lead.VestibularAxonID]));
    tempLagIDs = find(ismember(AugDBX(i).Inputs, [Lag.VestibularAxonID]));
    AugDBX(i).LeadVestibularPathLength = AugDBX(i).PathLength(tempLeadIDs)/max(Pvec_tree(AugDBX(i).Tree{1}));
    AugDBX(i).LeadVestibularPSD = AugDBX(i).PSDsize(tempLeadIDs);
    LeadVestibularPathLengths(i,:) = histcounts(AugDBX(i).LeadVestibularPathLength,0:0.1:1);
    
    AugDBX(i).LagVestibularPathLength = AugDBX(i).PathLength(tempLagIDs)/max(Pvec_tree(AugDBX(i).Tree{1}));
    AugDBX(i).LagVestibularPSD = AugDBX(i).PSDsize(tempLagIDs);
    LagVestibularPathLengths(i,:) = histcounts(AugDBX(i).LagVestibularPathLength,0:0.1:1);    
...
subplot(4,4,6)
histogram(vertcat(AugDBX.LeadVestibularPathLength),10,'FaceColor',leadColor,'EdgeColor','none');
hold on;
histogram(vertcat(AugDBX.LagVestibularPathLength),10,'FaceColor',lagColor,'EdgeColor','none');
axis square;
box off;
xlabel('Norm. pathlength');
ylabel('count')
...
UniqueIntegratorAxons = unique(vertcat(AugDBX.Integrator));
    ix1 = 1;
    ix2 = 1;  
...
for i = 1:numel(AugmentedCells)
    tempLeadIDs = find(ismember(AugDBX(i).Inputs, [Lead.IntegratorAxonID]));
    tempLagIDs = find(ismember(AugDBX(i).Inputs, [Lag.IntegratorAxonID]));
    AugDBX(i).LeadIntegratorPathLength = AugDBX(i).PathLength(tempLeadIDs)/max(Pvec_tree(AugDBX(i).Tree{1}));
    AugDBX(i).LeadIntegratorPSD = AugDBX(i).PSDsize(tempLeadIDs);
    LeadIntegratorPathLengths(i,:) = histcounts(AugDBX(i).LeadIntegratorPathLength,0:0.1:1);
    
    AugDBX(i).LagIntegratorPathLength = AugDBX(i).PathLength(tempLagIDs)/max(Pvec_tree(AugDBX(i).Tree{1}));
    AugDBX(i).LagIntegratorPSD = AugDBX(i).PSDsize(tempLagIDs);
    LagIntegratorPathLengths(i,:) = histcounts(AugDBX(i).LagIntegratorPathLength,0:0.1:1);    
...
subplot(4,4,10)
histogram(vertcat(AugDBX.LeadIntegratorPathLength),10,'FaceColor',leadColor,'EdgeColor','none');
hold on;
histogram(vertcat(AugDBX.LagIntegratorPathLength),10,'FaceColor',lagColor,'EdgeColor','none');
axis square;
box off;
xlabel('Norm. pathlength');
ylabel('count')
...
UniqueContraAxons = unique(vertcat(AugDBX.Contra));
    ix1 = 1;
    ix2 = 1;  
...
for i = 1:numel(AugmentedCells)
    tempLeadIDs = find(ismember(AugDBX(i).Inputs, [Lead.ContraAxonID]));
    tempLagIDs = find(ismember(AugDBX(i).Inputs, [Lag.ContraAxonID]));
    AugDBX(i).LeadContraPathLength = AugDBX(i).PathLength(tempLeadIDs)/max(Pvec_tree(AugDBX(i).Tree{1}));
    AugDBX(i).LeadContraPSD = AugDBX(i).PSDsize(tempLeadIDs);
    LeadContraPathLengths(i,:) = histcounts(AugDBX(i).LeadContraPathLength,0:0.1:1);
    
    AugDBX(i).LagContraPathLength = AugDBX(i).PathLength(tempLagIDs)/max(Pvec_tree(AugDBX(i).Tree{1}));
    AugDBX(i).LagContraPSD = AugDBX(i).PSDsize(tempLagIDs);
    LagContraPathLengths(i,:) = histcounts(AugDBX(i).LagContraPathLength,0:0.1:1);    
...
subplot(4,4,14)
histogram(vertcat(AugDBX.LeadContraPathLength),10,'FaceColor',leadColor,'EdgeColor','none');
hold on;
histogram(vertcat(AugDBX.LagContraPathLength),10,'FaceColor',lagColor,'EdgeColor','none');
axis square;
box off;
xlabel('Norm. pathlength');
ylabel('count')
...
UniqueEverythingElseAxons = unique(vertcat(AugDBX.EverythingElse));
    ix1 = 1;
    ix2 = 1;  
...
for i = 1:numel(AugmentedCells)
    tempLeadIDs = find(ismember(AugDBX(i).Inputs, [Lead.EverythingElseAxonID]));
    tempLagIDs = find(ismember(AugDBX(i).Inputs, [Lag.EverythingElseAxonID]));
    AugDBX(i).LeadEverythingElsePathLength = AugDBX(i).PathLength(tempLeadIDs)/max(Pvec_tree(AugDBX(i).Tree{1}));
    AugDBX(i).LeadEverythingElsePSD = AugDBX(i).PSDsize(tempLeadIDs);
    LeadEverythingElsePathLengths(i,:) = histcounts(AugDBX(i).LeadEverythingElsePathLength,0:0.1:1);
    
    AugDBX(i).LagEverythingElsePathLength = AugDBX(i).PathLength(tempLagIDs)/max(Pvec_tree(AugDBX(i).Tree{1}));
    AugDBX(i).LagEverythingElsePSD = AugDBX(i).PSDsize(tempLagIDs);
    LagEverythingElsePathLengths(i,:) = histcounts(AugDBX(i).LagEverythingElsePathLength,0:0.1:1);    
