function q=ashwinsVisualizer(fileName,colorString,newFigure,validNodeTypes)
...
% extract positions
x = [nodes(edges(:,1),2) nodes(edges(:,2),2)]';
y = [nodes(edges(:,1),1) nodes(edges(:,2),1)]';
z = [nodes(edges(:,1),3) nodes(edges(:,2),3)]';
q = [x(:) y(:) z(:)];
%plot3(q(:,1),q(:,2),q(:,3),'Color',colorString,'Marker','.','LineStyle','none');
for kk=1:size(edges,1) 
    % plot 3d with lines connnecting neighboring planes
