...
for i = 1:numel(cellIDs)
    if ismember(cellIDs(i), cellIDsAlx) == 1
        ...
        [A,B] =  UniqueSites30(allTrees,cellIDs,AxonTree,false,true);
        axonTree = zeros(size(A,1),3,Shuffles);
        axonTreeJitter = zeros(size(A,1),3,Shuffles);
        
        for j = 1:1:Shuffles;
            [axonTree(1:size(A,1),1:3,j), axonTreeJitter(1:size(A,1),1:3,j)]= UniqueSites30(allTrees,cellIDs,AxonTree,false,true);
        end
