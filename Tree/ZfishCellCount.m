%% Load images
ImageFname = 'ZFishLowResAlined1285.tif'; % file name for the stack used
...
SomaLocations = [ double(ZfishSmallX') , double(ZfishSmallY') , -1*double(ZfishSmallZ')];
...
ZFishVol = zeros( ksize + 393 + ksize , ksize +  750 + ksize , ksize + 1285 + ksize);

for n = 1:size(SomaLocations,1);
    ZFishVol(SomaLocations(n,1) + ksize/2 + (-ksize/2:ksize/2), SomaLocations(n,2)+ ksize/2 + (-ksize/2:ksize/2), SomaLocations(n,3) + ksize/2 + (-ksize/2:ksize/2) )= ...
        ZFishVol(SomaLocations(n,1) + ksize/2 + (-ksize/2:ksize/2), SomaLocations(n,2) + ksize/2 + (-ksize/2:ksize/2), SomaLocations(n,3) + ksize/2 + (-ksize/2:ksize/2))  + K3D ;
end

ZFishVol = ZFishVol(ksize : 393 + ksize , ksize : 750 + ksize, ksize : 1285 + ksize);
% project images

% XY projection
figure()
%subplot(2,1,1);
imageXY = zeros(size(ZFishVol,1), size(ZFishVol,2));

projectionDepth = 100;

if ~projectionDepth == 0
    
    for i = 1:projectionDepth
        imageXY = imageXY + ZFishVol(:,:,i);
    end
else
    for i = 1:size(ZFishVol,3)
        imageXY = imageXY + ZFishVol(:,:,i);
    end
end

h1 = imagesc(0,0,imageXY);
set(gca,'XDir','reverse');
%colorbar;
daspect([1,1,9]);
hold on;
...
for i = 1:length(Small)
    if ismember(i,SmallAlx) ==1
        scatter(Small(i,2),Small(i,1),300, calx,'filled','MarkerEdgeColor','w');
        set(gca, 'XDir','reverse');
    elseif ismember(i, SmallTrans) ==1
        scatter(Small(i,2),Small(i,1),300, ctrans,'filled','MarkerEdgeColor','w');
        set(gca, 'XDir','reverse');
    elseif ismember( i, SmallDbx) ==1
        scatter(Small(i,2),Small(i,1),300, cdbx,'filled','MarkerEdgeColor','w');
        set(gca, 'XDir','reverse');
    else
        scatter(Small(i,2),Small(i,1),300, cbarhl,'filled','MarkerEdgeColor','w');
        set(gca, 'XDir','reverse');
    end
end
...
peaksXY = sum(imageXY,2);
clear {'a','h1','h2','hMarkers'};
hold on;
...
for i = 1:length(Small)
    if ismember(i,SmallAlx) ==1
        [a,h1,h2] = plotyy(1:394,peaksXY',Small(i,1),Small(i,3),'line','scatter');
        set(h1,'LineStyle','none');
        set(h2,'MarkerFaceColor',calx,'MarkerEdgeColor','k', 'SizeData',500);
        set(a(2), 'XDir','reverse', 'YDir', 'reverse','YLim',[0,550], 'YTick',[], 'XTick',[]);
        hold on;
    elseif ismember(i, SmallTrans) ==1
        [a,h1,h2] = plotyy(1:394,peaksXY',Small(i,1),Small(i,3),'line','scatter');
        set(h1,'LineStyle','none');
        set(h2,'MarkerFaceColor',ctrans,'MarkerEdgeColor','k', 'SizeData',500);
        set(a(2), 'XDir','reverse', 'YDir', 'reverse','YLim',[0,550], 'YTick',[], 'XTick',[]);
        hold on;
    elseif ismember( i, SmallDbx) ==1
        [a,h1,h2] = plotyy(1:394,peaksXY',Small(i,1),Small(i,3),'line','scatter');
        set(h1,'LineStyle','none');
        set(h2,'MarkerFaceColor',cdbx,'MarkerEdgeColor','k', 'SizeData',500);
        set(a(2), 'XDir','reverse', 'YDir', 'reverse','YLim',[0,550], 'YTick',[], 'XTick',[]);
        hold on;
    else
        [a,h1,h2] = plotyy(1:394,peaksXY',Small(i,1),Small(i,3),'line','scatter')
        set(h1,'LineStyle','none');
        set(h2,'MarkerFaceColor',cbarhl,'MarkerEdgeColor','k', 'SizeData',500);
        set(a(2), 'XDir','reverse', 'YDir', 'reverse','YLim',[0,550], 'YTick',[], 'XTick',[]);
        hold on;
    end
end
[a,h1,h2] = plotyy(1:394,peaksXY',0,0,'line','scatter');
...
%         set(h1,'LineStyle','none');
%         set(h2,'MarkerFaceColor',calx,'MarkerEdgeColor','k');
%         set(a(2), 'XDir','reverse', 'YDir', 'reverse','YLim',[0,550], 'YTick',[], 'XTick',[]);
%         set(a(1),'OuterPosition',[0.47,0.37,0.1,0.2]);
%         set(a(2),'OuterPosition',[0.47,0.37,0.1,0.2]);
%         hold on;
%     elseif ismember(i, SmallTrans) ==1
%         [a,h1,h2] = plotyy(1:386,peaksXY',Small(i,1),Small(i,3),'line','scatter');
%         set(h1,'LineStyle','none');
%         set(h2,'MarkerFaceColor',ctrans,'MarkerEdgeColor','k');
%         set(a(2), 'XDir','reverse', 'YDir', 'reverse','YLim',[0,550], 'YTick',[], 'XTick',[]);
%         set(a(1),'OuterPosition',[0.47,0.37,0.1,0.2]);
%         set(a(2),'OuterPosition',[0.47,0.37,0.1,0.2]);
%         hold on;
...
%         [a,h1,h2] = plotyy(1:386,peaksXY',Small(i,1),Small(i,3),'line','scatter');
%         set(h1,'LineStyle','none');
%         set(h2,'MarkerFaceColor',cdbx,'MarkerEdgeColor','k');
%         set(a(2), 'XDir','reverse', 'YDir', 'reverse','YLim',[0,550], 'YTick',[], 'XTick',[]);
%         set(a(1),'OuterPosition',[0.47,0.37,0.1,0.2]);
%         set(a(2),'OuterPosition',[0.47,0.37,0.1,0.2]);
%         hold on;
...
%         [a,h1,h2] = plotyy(1:386,peaksXY',Small(i,1),Small(i,3),'line','scatter')
%         set(h1,'LineStyle','none');
%         set(h2,'MarkerFaceColor',cbarhl,'MarkerEdgeColor','k');
%         set(a(2), 'XDir','reverse', 'YDir', 'reverse','YLim',[0,550], 'YTick',[], 'XTick',[]);
%         set(a(1),'OuterPosition',[0.47,0.37,0.1,0.2]);
%         set(a(2),'OuterPosition',[0.47,0.37,0.1,0.2]);
%         hold on;
...
% set(h1,'LineStyle','-');
% set(h2,'MarkerFaceColor','none','MarkerEdgeColor','none');
% set(a(1),'XDir','reverse', 'YDir', 'reverse');%, 'XTick', 0.293*[0, 100, 200, 300, 400]);
% set(a(2), 'XDir','reverse', 'YDir', 'reverse','YLim',[0,550], 'YTick',[], 'XTick',[]);
% set(a(1),'OuterPosition',[0.47,0.37,0.1,0.2]);
% set(a(2),'OuterPosition',[0.47,0.37,0.1,0.2]);
