...
% C is the interpolated eye df/f; raw trace
...
[m,n] = sort(corr(cell2mat(fits),eyePos'));
...
for i = 1:Cells
    subplot(8,8,i)
    yyaxis left
    plot(time, C{n(i)}/max(C{n(i)}), 'Color','r', 'LineWidth',1.5);
    hold on;
    plot(time, fits{n(i)}/max(fits{n(i)}),'color',cols(i,:), 'LineWidth',1.5);% col);
    yyaxis right;
    plot(time, 1+eyePos./max(eyePos),'Color','k','LineWidth',1.5);
    axis off
end
...
%     plot(time, i+ (C{n(i)}),'Color', [col,0.7], 'LineWidth',1.5);
%     hold on;
...
% 
% [~,locU] = findpeaks(diff(eyePos),'MinPeakHeight',5);
% [~,locD] = findpeaks(diff(-1*eyePos),'MinPeakHeight',5);
% 
% xU = [locU locU];
...
% subplot(1,2,2)
% for i = 1:Cells
%     plot(time, i+fits{n(i)},'color',cols(i,:), 'LineWidth',1.5);% col);
%     hold on;
%     text (max(time)+10, i, num2str(n(i)));
%     axis off;
% end
